...
\begin{itemize}
    ...
    \item \textit{Ramp-up} and \textit{Ramp-down costs}: costs due to the fact \mywarning{Needs verification}
...
\mywarning{Talk about MTS option}
...
For the storage units, one needs some more parameters, given on Table \ref{table:plant-storage-db}. For the other unit types, these feilds will be left empty.
...
\begin{itemize}
    ...
    \item The net transfer capacities (NTC) between the different zones are given as inputs as hourly times series over a year. Then the maximum is picked and it is assumed that it remains constant over the year. \mywarning{Needs verification}
